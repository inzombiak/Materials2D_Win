...
void Game::ProcessEvents()
{
	...
	while (m_renderWindow.pollEvent(event))
	{
		if (event.type == sf::Event::Closed)
			...
			MousePressEvent(glm::vec2(event.mouseButton.x, event.mouseButton.y));
		else if (event.type == sf::Event::MouseMoved)
			MouseMoveEvent(glm::vec2(event.mouseMove.x, event.mouseMove.y));
		else if (event.type == sf::Event::MouseButtonReleased)
			MouseReleaseEvent(glm::vec2(event.mouseButton.x, event.mouseButton.y), false);
